...
_RE_COLLECTING_DEPENDENCY = re.compile(
    r'Collecting ([+a-zA-Z_\-.():/0-9>=<;,"]+)')  # Ignore PycodestyleBear (E501)
_RE_COLLECTING_DEPENDENCY_FROM = re.compile(r'Collecting ([+a-zA-Z_\-.():/0-9>=<;,"]+) '  # Ignore PycodestyleBear (E501)
                                            r'\(from ([a-zA-Z_\-.():/0-9>=<, ]+)\)')  # Ignore PycodestyleBear (E501)
_RE_COLLECTING_DEPENDENCY_REMOTE = re.compile(r'Collecting ([+a-zA-Z_\-.():/0-9>=<;,"]+) '  # Ignore PycodestyleBear (E501)
                                              r'from ([a-zA-Z_\-.():/0-9>=<, ]+)')  # Ignore PycodestyleBear (E501)
_RE_DOWNLOADING_ARTIFACT = re.compile(
    r'  Downloading ([+a-zA-Z_\-.:/0-9>=<;,"]+)( \(([a-zA-Z.,0-9]+)\))?')  # Ignore PycodestyleBear (E501)
_RE_ALREADY_SATISFIED = re.compile(r'Requirement already satisfied: ([+a-zA-Z_\-.():/0-9>=<;,"]+) in '  # Ignore PycodestyleBear (E501)
                                   r'([+a-zA-Z_\-.():/0-9>=<;,"]+) \(from ([a-zA-Z_\-.():/0-9>=<, ]+)\)')  # Ignore PycodestyleBear (E501)
...
class PIP3(HandlerBase):
    ...
    def _do_parse_package(package_specifier: str) -> typing.Tuple[str, typing.Optional[str]]:
        ...
        if package_specifier.startswith('git+'):
            _LOG.warning(
                "Detected installing a Python package from a git repository: %r", package_specifier)  # Ignore PycodestyleBear (E501)
    ...
    def _check_entry(result, package_name, package_version):
        ...
        if len(matched) > 1:
            _LOG.warning('Package %r was installed multiple times in versions %s', package_name,  # Ignore PycodestyleBear (E501)
                         tuple(entry['version_specified'] for entry in matched))  # Ignore PycodestyleBear (E501)
    ...
    def _parse_artifact(line: str) -> typing.Optional[dict]:
        ...
        if not match_result:
            _LOG.warning(
                'Unable to parse downloaded artifact from line %r', line)
            return None
